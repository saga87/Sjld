...
$("#userrolercombobox").combobox({  
    multiple:true,
    url:'wkrjsystem/wkrjRole/getAllRoleList'
}); 
...
function showAllUser(){
	...
	$("#userDeptTree").tree({url:'wkrjsystem/wkrjDept/getDeptTree'});//让选中的不选中
	$("#searchUserDept").combotree('clear');
...
function delUser() {
	...
	if (row) {
		...
		$.messager.confirm('删除提示', '你确定要删除这些数据吗,删除后不能回复请谨慎操作!!!', function(r) {
			if (r) {
				$.post('wkrjsystem/wkrjUser/delUser', {
					id : ids.join(",")
				}, function(result) {
...
function forbiddenUser() {
	...
	if (row) {
		$.messager.confirm('删除提示', '你确定要禁用这些用户吗', function(r) {
			if (r) {
				$.post('wkrjsystem/wkrjUser/forbiddenUser', {
					id : row.user_id,
					flag:1
				}, function(result) {
...
function repeatPw(){
	...
	if (row) {
		$.messager.confirm('删除提示', '你确定要重置密码吗,重置后密码变为123', function(r) {
			if (r) {
				$.post('wkrjsystem/wkrjUser/repeatPw', {
					id : row.user_id
				}, function(result) {
